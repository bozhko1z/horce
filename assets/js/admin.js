...
(function () {
    ...
    function branch() { return getSettings().branch || "main"; }
  
    function ghGetFile(path) {
      return fetch(ghApiBase() + path + "?ref=" + encodeURIComponent(branch()), { headers: ghHeaders() }).then(function (res) {
    ...
    function initSettings() {
      ...
      document.getElementById("btn-save-settings").addEventListener("click", function () {
        ...
        saveSettings(s);
        alertBox(statusEl, "success", "Запазено в този браузър. Натиснете „Тествай връзката“, за да проверите.");
        refreshLists();
      });
  
      document.getElementById("btn-clear-settings").addEventListener("click", function () {
        clearSettings();
        fillSettingsForm();
        alertBox(statusEl, "info", "Запазените данни са изчистени от този браузър.");
      });
      ...
      document.getElementById("btn-test-connection").addEventListener("click", function () {
        ...
        ghGetFile("data/concerts.json")
          .then(function () { alertBox(statusEl, "success", "Връзката работи. Може да добавяте съдържание."); })
          .catch(function (err) { alertBox(statusEl, "error", "Грешка: " + err.message + ". Проверете потребител, хранилище, branch и токен."); });
      });
    }
    ...
    function initConcertForm() {
      ...
      form.addEventListener("submit", function (e) {
        ...
        if (!s.owner || !s.repo || !s.token) {
          alertBox(statusEl, "error", "Първо запазете връзката с GitHub по-горе.");
          return;
        }
        var date = document.getElementById("c-date").value;
        var titleBg = document.getElementById("c-title-bg").value.trim();
        if (!date || !titleBg) { alertBox(statusEl, "error", "Попълнете поне дата и заглавие на български."); return; }
  
        var entry = {
          id: "c" + Date.now(),
          date: date,
          title: { bg: titleBg, en: document.getElementById("c-title-en").value.trim() },
          venue: { bg: document.getElementById("c-venue-bg").value.trim(), en: document.getElementById("c-venue-en").value.trim() },
          description: { bg: document.getElementById("c-desc-bg").value.trim(), en: document.getElementById("c-desc-en").value.trim() },
          photos: []
        };
        ...
        var uploadChain = files.reduce(function (chain, file, i) {
          return chain.then(function () {
            return readFileAsBase64(file).then(function (b64) {
              var path = "assets/images/concerts/" + slug + "/" + (i + 1) + "-" + safeFileName(file.name);
              return ghPutFile(path, b64, "Снимка за концерт: " + titleBg).then(function () {
        ...
        uploadChain
          .then(function () { return getJsonArray("data/concerts.json"); })
          .then(function (current) {
            var items = current.items;
            items.unshift(entry);
            return putJsonArray("data/concerts.json", items, current.sha, "Нов концерт: " + titleBg);
          })
          .then(function () {
            alertBox(statusEl, "success", "Концертът е запазен. Презаредете страницата „Концерти“ на сайта, за да го видите (може да отнеме до минута, докато GitHub Pages се обнови).");
            form.reset();
    ...
    function initEventForm() {
      ...
      form.addEventListener("submit", function (e) {
        ...
        var entry = {
          id: "e" + Date.now(),
          date: date,
          title: { bg: titleBg, en: document.getElementById("e-title-en").value.trim() },
          venue: { bg: document.getElementById("e-venue-bg").value.trim(), en: document.getElementById("e-venue-en").value.trim() },
          description: { bg: document.getElementById("e-desc-bg").value.trim(), en: document.getElementById("e-desc-en").value.trim() },
          poster: ""
        };
        var slug = date + "-" + slugify(titleBg);
        ...
        var posterStep = posterFile
          ? readFileAsBase64(posterFile).then(function (b64) {
              var path = "assets/images/events/" + slug + "/" + safeFileName(posterFile.name);
              return ghPutFile(path, b64, "Плакат за обява: " + titleBg).then(function () { entry.poster = path; });
            })
          : Promise.resolve();
  
        posterStep
          .then(function () { return getJsonArray("data/events.json"); })
          .then(function (current) {
            var items = current.items;
            items.push(entry);
            return putJsonArray("data/events.json", items, current.sha, "Нова обява: " + titleBg);
          })
          .then(function () {
            alertBox(statusEl, "success", "Обявата е публикувана. Презаредете страницата „Обяви“ на сайта, за да я видите (може да отнеме до минута).");
            form.reset();
      ...
      getJsonArray(path)
        .then(function (current) {
          ...
          container.innerHTML = current.items
            .map(function (item) {
              return (
                '<div class="entry-row"><span>' + (item.date || "") + " — " + ((item.title && item.title.bg) || "") +
                '</span><button type="button" data-id="' + item.id + '" data-path="' + path + '">Изтрий</button></div>'
              );
            })
            .join("");
          container.querySelectorAll("button[data-id]").forEach(function (btn) {
            btn.addEventListener("click", function () {
              if (!confirm("Да изтрия този запис от сайта? (Снимковите файлове в хранилището няма да бъдат изтрити автоматично.)")) return;
              getJsonArray(path).then(function (current2) {
                var items = current2.items.filter(function (i) { return i.id !== btn.dataset.id; });
                return putJsonArray(path, items, current2.sha, "Изтриване на запис");
              }).then(function () {
        ...
        .catch(function (err) {
          container.innerHTML = '<p class="hint">Грешка при зареждане: ' + err.message + "</p>";
        });
    }
