...
(function () {
    ...
    function eventCard(item) {
      ...
      return (
        '<article class="post-card reveal">' +
        (item.poster
          ? '<div class="photo-strip" style="grid-template-columns:1fr;"><img src="' + escapeHtml(item.poster) + '" alt="" loading="lazy" data-full="' + escapeHtml(item.poster) + '"></div>'
          : "") +
        '<div class="body">' +
        (isUpcoming
...
document.addEventListener("click", function (e) {
  ...
  var description = card.querySelector("p") ? card.querySelector("p").innerHTML : "";
  var imageSrc = card.querySelector("img") ? card.querySelector("img").src : "";
  ...
  metaElements.forEach(function(el) {
    if(!el.classList.contains("more-details-btn")) {
      metaHtml += '<div class="' + el.className + '">' + el.innerHTML + '</div>';
    }
  });
  ...
  modalOverlay.querySelector(".info-modal-close").addEventListener("click", closeModal);
